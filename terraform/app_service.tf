...
resource "azurerm_windows_web_app_slot" "hro-management-api-staging-slot" {
  ...
  app_settings = {
    ...
    CAPITAOPTIONS__APIADDRESS       = "@Microsoft.KeyVault(SecretUri=${azurerm_key_vault_secret.capita-api-address-staging.id})"
    CAPITAOPTIONS__USERNAME         = "@Microsoft.KeyVault(SecretUri=${azurerm_key_vault_secret.capita-username-staging.id})"
    CAPITAOPTIONS__PASSWORD         = "@Microsoft.KeyVault(SecretUri=${azurerm_key_vault_secret.capita-password-staging.id})"
    CAPITAOPTIONS__STANDARDJOBCODE  = "@Microsoft.KeyVault(SecretUri=${azurerm_key_vault_secret.capita-standardjobcode-staging.id})"
    CAPITAOPTIONS__SOURCE           = "@Microsoft.KeyVault(SecretUri=${azurerm_key_vault_secret.capita-source-staging.id})"
    CAPITAOPTIONS__SUBLOCATION      = "@Microsoft.KeyVault(SecretUri=${azurerm_key_vault_secret.capita-sublocation-staging.id})"
  }
...
resource "azurerm_windows_web_app" "hro-management-api" {
  ...
  app_settings = {
    ...
    COSMOS_ENDPOINT_URL             = "@Microsoft.KeyVault(SecretUri=${azurerm_key_vault_secret.cosmos-endpoint-url.id})"
    COSMOS_AUTHORIZATION_KEY        = "@Microsoft.KeyVault(SecretUri=${azurerm_key_vault_secret.cosmos-authorization-key.id})"
    CAPITAOPTIONS__APIADDRESS       = "@Microsoft.KeyVault(SecretUri=${azurerm_key_vault_secret.capita-api-address-production.id})"
    CAPITAOPTIONS__USERNAME         = "@Microsoft.KeyVault(SecretUri=${azurerm_key_vault_secret.capita-username-production.id})"
    CAPITAOPTIONS__PASSWORD         = "@Microsoft.KeyVault(SecretUri=${azurerm_key_vault_secret.capita-password-production.id})"
    CAPITAOPTIONS__STANDARDJOBCODE  = "@Microsoft.KeyVault(SecretUri=${azurerm_key_vault_secret.capita-standardjobcode-production.id})"
    CAPITAOPTIONS__SOURCE           = "@Microsoft.KeyVault(SecretUri=${azurerm_key_vault_secret.capita-source-production.id})"
    CAPITAOPTIONS__SUBLOCATION      = "@Microsoft.KeyVault(SecretUri=${azurerm_key_vault_secret.capita-sublocation-production.id})"
  }
}
